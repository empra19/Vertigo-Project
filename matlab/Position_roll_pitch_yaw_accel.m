...
figure; %yaw
[xy,yy] = pol2cart(euldata_window(:,3)*2*pi/360,5); %converts from polar to cartersian co-ordinates
...
figure %pitch
[xp,yp] = pol2cart(euldata_window(:,1)*2*pi/360,5);
quiver (decimate(smooth_north_position, quiv_ds_rate), ...
    decimate(smooth_down_position, quiv_ds_rate), ...
    decimate(xp, 18), ...
    decimate(yp, 18));
...
figure %roll
[xr,yr] = pol2cart(euldata_window(:,2)*2*pi/360,5);
quiver (decimate(smooth_east_position, quiv_ds_rate), ...
    decimate(smooth_down_position, quiv_ds_rate), ...
    decimate(xr, 18), ...
    decimate(yr, 18));
...
quiver3 (decimate(smooth_east_position,20),decimate(smooth_north_position,20),...
    decimate(smooth_down_position,20),...
    decimate(data_accel_east,18),decimate(data_accel_north,18),...
    decimate(data_accel_down, 18), 1.2);

legend ('Acceleration(m/s^2)', 'Location', 'northeast')
xlabel ('East Position')
ylabel ('North Position')
zlabel ('Down Position')
axis image
