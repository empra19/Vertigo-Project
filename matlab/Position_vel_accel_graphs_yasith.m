...
% plot3(position_east_gps, position_north_gps, data_alt_gps);
% hold on
% comet3(kal_xe_stor(:,1), kal_xn_stor(:,1), kal_x_stor(:,1))

plot3(-kal_xe_stor(:,1), kal_xn_stor(:,1), kal_x_stor(:,1), -position_east_gps, position_north_gps, data_alt_gps);
legend('Fusion Position', 'GPS','Location', 'northwest');
title ('Ride Mapping')
xlabel('East (m)');
ylabel('North (m)');
zlabel('Altitude (m)');
...
title ('Acceleration at Position Graph');
...
quiver3 (-(decimate(positionsE, quiv_ds_rate)), ...
    ...
    decimate(down_vel_elements, quiv_ds_rate),ars);
hold on
plot3 (-(smooth_east_position),smooth_north_position,smooth_down_position);
legend('Velocity at position', 'Location', 'northeast');
title ('Velocity at Position Graph')
xlabel('East Postion(m)');
ylabel('North Position(m)');
zlabel('Down Position(m)');
axis image
...
title ('Position, Velocity, Acceleration Time Graph')
legend('Position', 'Velocity' , 'Acceleration', 'Location', 'southeast');
xlabel('Time(s)');
ylabel('m, m/s, m/s^2');
grid on
axis tight
...
accelNsq = north_accel_elements.^2;
accelEsq = east_accel_elements.^2;
accelDsq = down_accel_elements.^2;

accel_mag = sqrt (accelNsq + accelEsq + accelDsq);

figure;
plot (stotal_time, accel_mag/9.8);  % plots magnitude/ time
xlabel ('Time (s)')
ylabel ('Magnitude of G Force (g)')
title ('G Force')
...
figure;
plot(stotal_time, north_accel_elements/9.8)
hold on
plot(stotal_time, east_accel_elements/9.8)
hold on
plot(stotal_time, down_accel_elements/9.8)
...
velNsq = north_vel_elements.^2;
velEsq = east_vel_elements.^2;
velDsq = down_vel_elements.^2;
velmag = sqrt (velNsq + velEsq + velDsq)

figure
plot(stotal_time,velmag)
xlabel ('Time (s)')
ylabel ('Velocity (m/s)')
...
jerkd = diff(down_accel_elements)
jerkn = diff(north_accel_elements)
jerke = diff(east_accel_elements)


jtime = stotal_time(1:3852,:)
figure
plot (decimate(jtime,quiv_ds_rate), decimate(jerkn,quiv_ds_rate))
hold on 
plot(decimate(jtime,quiv_ds_rate), decimate(jerke,quiv_ds_rate));
hold on
plot(decimate(jtime,quiv_ds_rate), decimate(jerkd,quiv_ds_rate));
axis tight
grid on
xlabel ('Time (s)')
ylabel ('Jerk (m/s^3)')
