...
quiver_length = 5;
[pitchx,pitchy] = pol2cart(euldata_window(:,2)*2*pi/360,quiver_length);
[rollx,rolly] = pol2cart(euldata_window(:,1)*2*pi/360,quiver_length);
...
figure
plot( decimate(imudata(:,1),dr),...
    decimate(imudata(:,6),dr) );
hold on
plot( decimate(imudata(:,1),dr),...
    decimate(imudata(:,7),dr) );
hold on
plot( decimate(imudata(:,1),dr),...
    decimate(imudata(:,8),dr) );
hold off
xlabel('Time (s)');
